...
document.querySelectorAll('.contact-form').forEach(form => {
    form.addEventListener('submit', async function(e) {
        ...
        // Собираем только имя, телефон и согласие
        const formData = {
            name: this.querySelector('[name="name"]').value.trim(),
            phone: this.querySelector('[name="phone"]').value.trim(),
            consent: this.querySelector('#pd-consent') 
                ? this.querySelector('#pd-consent').checked 
                : true
        };
...
document.addEventListener('DOMContentLoaded', function() {
  // Находим все плашки
  const rightPanels = document.querySelectorAll('.page-right');
  
  rightPanels.forEach(panel => {
    // 1. Делаем саму плашку видимой
    panel.style.cssText = 'position:absolute; bottom:0; left:0; width:100%; min-height:110px; background:#0a0a0a; border-top:3px solid #ff7b00; z-index:999; display:flex; align-items:center; justify-content:center; opacity:1; visibility:visible;';
    
    // 2. Находим текст и красим его принудительно
    const h4 = panel.querySelector('h4');
    const p = panel.querySelector('p');
    
    if (h4) {
      h4.style.cssText = 'color:#ff7b00 !important; font-size:24px !important; font-weight:900 !important; display:block !important; opacity:1 !important; visibility:visible !important; margin-bottom:6px;';
      h4.textContent = h4.textContent || 'Имя'; // Защита от пустоты
    }
    
    if (p) {
      p.style.cssText = 'color:#ffffff !important; font-size:16px !important; display:block !important; opacity:1 !important; visibility:visible !important; margin:0;';
      p.textContent = p.textContent || 'Должность'; // Защита от пустоты
    }
  });
});
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  let currentIndex = 0; // Индекс текущей страницы
  ...
  nextBtn.addEventListener('click', function() {
    if (currentIndex < pages.length - 1) {
      ...
      // Если конец списка, можно вернуться в начало (зацикливание)
      currentIndex = 0;
      showPage(currentIndex);
    }
  });
  ...
  prevBtn.addEventListener('click', function() {
    if (currentIndex > 0) {
      ...
      // Если начало списка, идем в конец (зацикливание)
...
// === ПРОВЕРКА ГАЛОЧКИ ПЕРЕД ОТПРАВКОЙ ===
document.querySelectorAll('form').forEach(form => {
  form.addEventListener('submit', function(e) {
    // Находим чекбокс внутри этой формы
    const checkbox = this.querySelector('#pd-consent');
    
    // Если чекбокс есть И он не отмечен
    if (checkbox && !checkbox.checked) {
      e.preventDefault(); // Останавливаем отправку
      alert('Пожалуйста, подтвердите согласие на обработку данных.');
      
      // Визуальная подсказка (тряска или подсветка)
